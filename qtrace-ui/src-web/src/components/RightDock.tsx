...
export function RightDock({ row, detail, registers, memory, history, annotation, errors, workspaceId, artifactIndex, completeness, onSaveComment, onDeleteComment, onSaveHighlight, onDeleteHighlight }: RightDockProps) {
  return <aside className="right-dock" aria-label="Event details"><h2>Details</h2><EventDetailPane detail={detail} /><CompletenessPane row={row} ranges={completeness} /><RegisterPane state={registers} /><MemoryPane state={memory} history={history} /><SymbolPane workspaceId={workspaceId} artifactIndex={artifactIndex} />{row !== null && <AnnotationEditor comment={annotation?.comment ?? ""} highlight={annotation?.highlight ?? ""} onSave={onSaveComment} onDelete={onDeleteComment} onSaveHighlight={onSaveHighlight} onDeleteHighlight={onDeleteHighlight} />}{errors.map((error) => <p role="alert" key={`${error.stage}:${error.code}`}>{error.code}: {error.detail}</p>)}</aside>;
}
