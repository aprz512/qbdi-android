...
### Final-review finding matrix

| Finding | Initial RED / gap | Resolution / GREEN evidence |
|---|---|---|
| Critical: public shape baseline omitted direct resident work | The provider oracle compared allocator requests only with bytes submitted through `begin_allocation_scope`; a direct resident `WorkDelta` precharge could raise the real cumulative guard total without changing the asserted authorization total. A focused harness mutation consumes one direct resident byte and proves the totals differ. | Every successful baseline and exact-budget rerun now asserts `AllocationGuard::consumed == OracleState::authorized_bytes`. The authorization/request ratio and N/2N/4N measurements use that total consumed resident work, so an unscoped synthetic precharge cannot hide outside the ratio. Production contains no added precharge. |
| Critical: shape fixtures were not proven to decode as intended | `drain_provider` discarded every decoded event and the provider summary, so a fixture-name label could pass allocation checks after falling back to opaque/damaged output or decoding a different cardinality. | `drain_provider` returns all `EventRecord`s plus `ProviderSummary`. QTRB Read0/Read34/Write0/Write34 prove exact `(reads,writes)` values `(0,1)`, `(34,1)`, `(1,0)`, `(1,34)` in both definitions and observations, with exactly four events. Flight definition and string families prove exactly 8/16/32 typed events; fragment families prove 8/16/32 physical fragments become 4/8/16 `SemanticRule` events, with the three fixed baseline string definitions explicitly checked. Every shape proves summary/event cardinality agreement and zero opaque, damaged, or unexpected fallback output from decoded values rather than fixture names. |
| Important: Flight slope bound was too permissive | The prior public-path test accepted a second allocation slope up to three times the first and did not bind each family/count to the production recurrence. | For both total consumed authorization and complete allocator requests, the 2N→4N slope must now equal twice the N→2N slope within an explicit 512-byte alignment/hash allowance. Each definition/string/fragment family at 8/16/32 walks the exported production `checked_geometric_capacity` recurrence and proves terminal capacity `<2N`; the checked 10M production-planner test remains GREEN. |
| Important: shared allocation helpers erased Flight error identity | Focused tests initially failed to compile because shared helpers had no caller error context. Their hard-coded fallback was `control.resource_exhausted` at `provider.allocation`, even when a Flight open/recovery geometric overflow or reserve failed. | Shared helpers now accept an `AllocationErrorContext`. Flight open passes `source.flight.resource` / `flight.open`; recovery, typed decoding, and fragment assembly pass `source.flight.resource` / `flight.recovery` through thin adapters. Focused geometric-overflow and impossible-reserve tests assert the exact code/stage. Default QTRB behavior remains `control.resource_exhausted` / `provider.allocation`, while guard aborts remain their original typed control errors. |
...
- `cargo test -p qtrace-store` — 143 passed: library 39, allocation authorization 7, cache
  ...
  open 20.
- `cargo test -p qtrace-provider` — 134 passed, 1 intentional ignored child entry: library 9,
  Flight differential 2/events 5/recovery 39, provider allocation shapes 3, model 23,
