...
// uintptr_t and size_t values are widened to u64 on the wire; the stream header records the source
// pointer width so a decoder can validate the producer ABI.
...
// TRACE_BEGIN payload: module_base u64, target_offset u64, target_address u64,
// effective_buffer_bytes u64, pid u32, tid u32, scene string, target string, package string.
inline constexpr size_t kBinaryTraceBeginFixedPayloadBytes = 46;
inline constexpr size_t kBinaryMaxTraceBeginRecordBytes =
        kBinaryRecordHeaderBytes + kBinaryTraceBeginFixedPayloadBytes +
        3U * kBinaryMaxContextStringBytes;
...
// CALL/RULE/ERROR payload: name string, detail string.
inline constexpr size_t kBinaryEventFixedPayloadBytes = 4;
inline constexpr size_t kBinaryMaxEventRecordBytes =
        kBinaryRecordHeaderBytes + kBinaryEventFixedPayloadBytes +
...
static_assert(kBinaryMaxTraceBeginRecordBytes == 819);
...
static_assert(kBinaryMaxEventRecordBytes == 4363);
