...
std::string trace_filename(const TraceContext &context, bool compressed, uint64_t sequence) {
    ...
    return std::to_string(millis) + "_" + std::to_string(context.pid) + "_" +
           std::to_string(context.tid) + "_" + context.scene_name + "_0x" + offset + "_" +
           std::to_string(sequence) + (compressed ? ".trace.bin.lz4" : ".trace.bin");
}
...
TraceMetrics producer_metrics_snapshot(const TraceMetrics &metrics) {
    ...
    snapshot.compressed_bytes = metrics.compressed_bytes;
...
BinaryTraceWriter::BinaryTraceWriter(const TraceOptions &options, TraceMetrics *metrics,
    ...
    : options_(options), metrics_(metrics), dictionary_(), writer_(backend, faults),
      faults_(faults), path_(new (std::nothrow) std::string) {}
...
const std::string &BinaryTraceWriter::path() const {
...
bool BinaryTraceWriter::prepare(const TraceContext &context) {
    ...
    const std::string filename = trace_filename(context, options_.compression_enabled, sequence);
...
bool BinaryTraceWriter::open_prepared() {
    ...
    *metrics_ = {};
    ...
    if (!writer_.open(*path_, options_, metrics_)) {
        QTRACE_E("open binary trace file failed: %s", path_->c_str());
...
bool BinaryTraceWriter::end(uint64_t retval, bool ok, long elapsed_ms) {
    ...
    WritableSpan span = writer_.reserve(kBinaryTraceEndRecordBytes);
    if (span.data == nullptr) return fail();
    ...
    const BinaryEncodeResult result = encoder_.encode_end(
            reinterpret_cast<uint8_t *>(span.data), span.capacity, ok, retval, elapsed_ms_,
            footer_metrics);
    if (!result.ok || result.size != kBinaryTraceEndRecordBytes) {
        writer_.commit(0);
        return fail(EINVAL);
    }
    writer_.commit(result.size);
...
bool BinaryTraceWriter::close() {
    ...
    if ((!trace_ok || !metrics_ok) && path_ != nullptr)
        (void)::unlink((*path_ + ".metrics").c_str());
