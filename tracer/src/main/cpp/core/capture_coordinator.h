...
public:
    ...
    bool start(const TraceConfig &config, const ModuleRange &module,
    ...
    void mark_coverage_gap(uint32_t tid, uintptr_t pc) noexcept;
    ...
    uint64_t run_id() const noexcept { return run_id_; }
    uint32_t module_generation() const noexcept { return module_generation_; }
    const ModuleRange &module() const noexcept { return module_; }
...
private:
    ...
    void mark_coverage_gap_locked(uint32_t tid, uintptr_t pc) noexcept;
